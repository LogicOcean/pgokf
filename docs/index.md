...
| **Multi-tenancy (opt-in)** | Row-level security keyed on the `pgokf.tenant` session GUC isolates tenants' bundles; a session that sets no tenant sees everything (or nothing, with the `require_tenant` policy), so existing installs are unaffected. See [Multi-tenancy](multi-tenancy.md). |
