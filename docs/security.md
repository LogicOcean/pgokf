...
real login user; nobody logs in *as* them. A fourth role,
**`pgokf_dispatcher`**, stands outside the hierarchy: it may only claim and
acknowledge delivery of catalog change events
(`claim_catalog_change_events` / `ack_catalog_change_event`) and is the intended
account for an event-delivery worker - it cannot search, ingest, or configure,
and no tier inherits it.

| Role | Capabilities |
| ---- | ------------ |
| `pgokf_reader` | `USAGE` on schema `pgokf`; `SELECT` on the projection tables (including `concept_source`, `concept_embedding`, `concept_history`, and `bundle_log`); `EXECUTE` on the whole read surface: search (`concept_search`, `concept_search_fresh`, `search_facets`, `find_similar`, `concept_search_semantic`, `concept_search_hybrid`, `search_index_status`), graph traversal (`concept_neighbors`, `concept_relationship_neighbors`), catalog reads (`list_bundles`, `bundle_info`, `list_bundle_log`, `duplicate_concepts`), the capability declaration (`capabilities`), and the freshness/relationship projections (`effective_freshness`, `current_relationships`), monitoring and audit reads (`catalog_stats`, `health`, `stale_concepts`, `list_sync_log`, `list_sync_changes`), version history (`concept_history`, `concept_as_of`), plus `get_config` and `get_concept_source`. Where the external repository-registry producer service shares the database, it also gets `USAGE` on the `ast_graph` schema and a column-narrowed `SELECT` on `ast_graph.repository_registry` (the registry's listable columns only - never `checkout_path` or the producer's internal `graph_id`; fetch credentials are never in this table at all). |
| `pgokf_writer` | Inherits `pgokf_reader` (it is `GRANT`ed the reader role), plus `EXECUTE` on the **ingestion and lifecycle** mutators: `register_bundle`, `register_bundle_content` (the mountless content-ingestion API, which streams object-store bytes into the catalog with no filesystem mount), `refresh_bundle`, `unregister_bundle`, `set_bundle_enabled`, `retire_bundle` / `unretire_bundle`, and `set_concept_embedding` (the vector-streaming half of semantic search). It is also the **producer tier** for the generic capability APIs: freshness dependency registration (`register_freshness_dependency`, `disable_freshness_dependency`, `remove_freshness_dependency`), freshness state transitions (`mark_stale`, `mark_scope_stale`, `mark_reconciling`, `mark_blocked`, the compare-and-set `mark_fresh`, `clear_freshness_scope`), publication fencing (`issue_publication_fence`, `release_publication_fence`), the provenance-checked `set_concept_embedding_cas`, and the generation-bound `replace_relationships`. It cannot change configuration, write exports, purge retired bundles, rebuild indexes, read `pgokf_private`, or claim/acknowledge change events (that is `pgokf_dispatcher`'s sole purpose). It cannot change configuration, write exports, purge retired bundles, rebuild indexes, or read `pgokf_private`. This is the intended account for an automated ingestion pipeline and for the `pgokf-ingest` / `pgokf-embed` companions. |
| `pgokf_admin` | Inherits `pgokf_writer` (and thus `pgokf_reader`), plus `USAGE` on `pgokf_private` and `EXECUTE` on the admin-only surface: configuration (`set_config`, `reset_config`), the file-writing exports (`export_parquet`, `export_sources`), the hard delete of retired bundles (`purge_retired`), the index rebuilds (`rebuild_search_index`, `rebuild_embedding_index`), the `pg_cron` adapter (`schedule_refresh`, `unschedule_refresh`), the external repository-registry writers (`registry_set_status`, `registry_set_poll_interval`), and the exfiltration audit (`list_access_log`). |
