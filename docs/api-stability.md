...
| `pgokf.registry_set_status(uuid, text)` | `pgokf_admin` | Pause/resume one registered repository of the external repository-registry producer service (`ast_graph.repository_registry.status`, `active`/`paused`; runtime-only coupling, curated `22023` when the producer schema is absent; tenant-confined: a cross-tenant id earns the same `22023` as an unknown one) |
