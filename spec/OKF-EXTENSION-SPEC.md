...
| `author` | string or mapping | yes | The publisher or owner of the concept, in the OKF actor convention: `human:<id>` for a person, `process:<id>` for a pipeline, `<producer>/<version>` for an agent or tool as OKF writes it, plus one pgokf extension prefix, `agent:<id>`, for an agent identified by name rather than producer/version (it is treated exactly like a non-human OKF actor for trust tiering). No other prefix is defined; a team is expressed with owner-style custom metadata (for example `owner: team-platform`), never as an actor. A mapping MAY add display metadata around the same actor string. `author` does not replace OKF `generated`, which records who produced the *current* content and is REQUIRED on typed concepts (the `human:` prefix there drives the derived trust tier). |
...
### 11.3 Companion transports and bearer authorization

A companion's authority is its database role, so a transport that carries requests from somewhere else MUST decide who may use it. Where the client starts the companion itself (the agent-facing server over stdio) it already holds the connection string and there is nothing to authenticate. Where the companion listens on a socket, every request MUST carry a credential, and the credential's role MUST decide what it may reach.

For the agent-facing server this is a bearer token. Tokens are high-entropy secrets minted by the companion (at least 128 bits from the system random source, with a recognizable prefix), never chosen by a person; only a digest is stored (a digest is not the credential: learning one, by timing or otherwise, yields no token), and a fast hash is correct precisely because the token is not a password — so the companion MUST check that a presented credential has the shape it mints *before* hashing it, making that assumption structural rather than aspirational. The token registry is the catalog itself (`pgokf_web.mcp_tokens`, a table only the writer tier may see): the web companion mints a token as an admin's action and stores its digest, and the agent-facing server — a reader — authenticates by hashing the presented credential and asking one `SECURITY DEFINER` lookup for the bearer of that digest, which answers for the digest it is given and lists nothing, so the credential never travels to the database and a reader learns nothing about tokens it does not hold. Every request is asked afresh, so revoking a token takes effect with the next request and a revocation cannot fail silently. A token names the tenant it was minted for, and a server admits only tokens minted for the tenant it serves, so tokens for one tenant's endpoint open no other's on a shared catalog. The lookups on the request path MUST run on a connection of their own, so that authentication never waits behind work, and MUST be bounded independently of the work slots, so that a flood of wrong credentials costs the catalog a fixed amount; the server MUST prove the lookup exists before it opens a socket on the strength of it, and its health endpoint MUST prove it again; while the catalog cannot be asked, a request is answered with a retryable failure rather than a refusal; and refusals and failures MUST be summarized into the log rather than written per request, so that a flood of them cannot become the denial of service itself. A credential MUST be read only from the request's authorization header, never from a URL, and MUST NOT appear in any log; a name or tool echoed into a log MUST be bounded and escaped, because the caller chose it.

Authority is a role, and one predicate MUST decide it: the tool listing and the tool call consult the same answer, so a client is never offered a capability it may not use, and an unknown capability is refused rather than allowed by omission. Initial roles for the agent-facing server: `reader` (search and read the catalog) and `builder` (also materialize a workspace plugin, which reads every selected source through the audited readers). Arguments that would act on the *server's* host rather than the caller's — writing a materialized tree into a directory — MUST be refused over a network transport, and the tree returned to the caller instead; the set of such arguments MUST be declared beside the tool's own schema and derived from it, never kept as a second list in the transport, so a new one is refused the day it is added. A capability the caller may not use MUST NOT be listed to it, and neither MUST an argument it may not pass.

A network transport MUST also refuse a request that carries a browser origin the operator has not named (the defence against a page in a browser reaching a companion on a private network), and MUST authenticate **before** it reads a request body or takes one of its working slots, so that an unauthenticated caller can neither buffer nor queue anything. It MUST bound request bodies, how long a body may take to arrive, concurrency, wall-clock time per request, and the time a database statement may run — cancelling a request does not cancel its query. A companion whose database link cannot be re-established MUST stop when that link closes rather than answer every later call with the same failure. It MUST NOT terminate TLS itself: it is bound to a loopback or private interface with a terminating proxy in front. One process serves one tenant: the tenant scope belongs to the session, not to a credential.
...
- metadata: string/object author/origin, the OKF actor convention plus the `agent:` extension prefix (no other undefined prefix), forbidden secret keys, unknown extension keys, OKF provenance coexistence;
...
An adapter's MCP configuration MAY describe either a server the harness starts or a **remote endpoint it calls** (§11.3), and the remote form is documentation like the local one: read from the harness's own, recorded with its source, and refused where it has not been read. Whichever it is, no secret enters the tree. A connection string is referenced through the harness's expansion form, forwarded, or left as a placeholder in a file outside the workspace; a bearer token is subject to exactly the same rule, and where a harness documents no way to reference one from a configuration file, the entry MUST NOT be written into that harness's own file at all — it is emitted as a fragment to merge, so that no file the build produces is intended to hold a credential. The manifest records which endpoint a tree was built for, and the guide states how the credential reaches the harness.

### 21.6 The human side: upload, edit, review

The web companion carries the human workflow the trust model presumes. It is off until the operator gives it a `pgokf_writer` connection and a way of knowing who is asking: an `OpenID` Connect provider the site signs people in against (authorization code flow with PKCE, the ID token verified against the provider's published keys), an authenticating proxy's identity headers, believed only from the proxy's own addresses, or people kept in the catalog (`pgokf_web.users`, writer-only) with a login form and a signed session whose liveness the catalog records (`pgokf_web.sessions`), so a session can be ended on every device at once; the bearer tokens the agent-facing server accepts (§11.3) are minted there too, by an admin, and kept as digests (`pgokf_web.mcp_tokens`); and an admin MAY set up an identity provider there (`pgokf_web.identity_provider`, one row: an `OpenID` Connect provider, or GitHub by its `OAuth` web flow with the person read from its API), which the sign-in page then offers beside the local one - its client secret stored only sealed under a key derived from the site's own session secret, so the catalog holds no credential usable at the provider. Anonymous requests never reach the writer. Roles are a ladder (`viewer`, `uploader`, `editor`, `approver`, `admin`), and each action is a person's: a person's OKF actor is `human:<name>`. The workflow acts on **content bundles** (§7.3 `register_bundle_content`), rebuilding the full snapshot from the sources the catalog keeps (`store_source` MUST be on); a bundle synced from a directory or a bucket is changed at its source. Every decision is an ordinary OKF field written into the document itself, so the derived trust tier and lifecycle status are the ones every reader, agent, and plugin sees: an upload stamps `generated` and `author` when absent; an edit sets earlier `verified` events aside under `superseded_verifications` (who, when, why) and names the editor in `generated`, so the document returns to review; an approval appends a `verified` event under the approver's actor (a draft becomes `active`); a send-back sets `status: draft` and keeps the note under `reviews`. A `verified` list carried by an upload or typed into an edit is set aside, never believed: verification is granted only by an approver.
