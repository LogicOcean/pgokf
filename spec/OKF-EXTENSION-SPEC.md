...
For the agent-facing server this is a bearer token. Tokens are high-entropy secrets minted by the companion (at least 128 bits from the system random source, with a recognizable prefix), never chosen by a person; only a digest is stored, compared in constant time, and a fast hash is correct precisely because the token is not a password — so the companion MUST check that a presented credential has the shape it mints *before* hashing it, making that assumption structural rather than aspirational. The token registry is an operator-owned file of `name:role:digest` lines whose digest is the terminal field, re-read when it changes (with a bound on how often, since the check is on the request path) so that removing a line revokes a token without a restart and an emptied registry revokes everyone. Any change MUST be noticed, not only a newer timestamp. A registry that stops reading is reported and the last good one retained only for a bounded grace period — long enough to ride out a half-written save — after which every request is refused: a revocation MUST NOT be able to fail silently, and the companion's health endpoint MUST report the condition throughout. A credential MUST be read only from the request's authorization header, never from a URL, and MUST NOT appear in any log; a name or tool echoed into a log MUST be bounded and escaped, because the caller chose it.
