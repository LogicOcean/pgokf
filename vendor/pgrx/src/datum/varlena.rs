...
use crate::{
    FromDatum, IntoDatum, PgMemoryContexts, pg_sys, rust_regtypein, set_varsize_4b,
    set_varsize_short, vardata_any, varsize_any, void_mut_ptr,
};
...
// pgokf vendoring delta (see VENDORING.md beside the vendored Cargo.toml):
// the cbor_encode/cbor_decode/cbor_decode_into_context helpers and the
// serde_cbor dependency are removed. They were #[doc(hidden)] internals used
// only by #[derive(PostgresType)] default storage functions, which nothing in
// the pgokf workspace uses.
