...
| `--producer-admin-url`, `--producer-admin-token` | `OKF_PRODUCER_ADMIN_URL`, `OKF_PRODUCER_ADMIN_TOKEN` | The repository-registry producer service's admin API: base URL and the static admin bearer token it requires, given together. They enable the Admin page's Registry tab credential controls (set/remove a per-repository git fetch credential); the registry table itself - projects, keys, branches, remotes, statuses, poll intervals, and publication evidence - is read from the catalog database either way, and pause/resume and poll-interval changes go through the writer connection (whose role must hold the `pgokf_admin` tier). The token is held server-side only, and a credential's secret transits exactly once (browser form to the producer): it is never persisted, logged, or rendered. A producer that does not answer surfaces an honest "unavailable" on the page, never a silent success. |
