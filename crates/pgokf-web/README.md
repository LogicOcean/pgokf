...
| **Search** (`/search`) | `concept_search()` (lexical, on the configured backend), `concept_search_semantic()` / `concept_search_hybrid()` when an embeddings endpoint is configured, `search_facets()` for the type, tag, bundle, status, and trust-tier facets, keyset pagination (a First-page / Next-page pager; results render as a table, with the filters behind a toggle bar on phones). With filters but no query text it **browses**: every visible concept matching the filters, so a tag, type, status, or bundle link lists concepts directly. |
...
| `--producer-admin-url` | `OKF_PRODUCER_ADMIN_URL`, `OKF_PRODUCER_ADMIN_TOKEN` | The repository-registry producer service's admin API: base URL and the static admin bearer token it requires, given together. They enable the Admin page's Registry tab repository registration (idempotent at the producer, with an optional first credential set in the same flow, so a private repository's first reconcile is already authenticated) and each repository page's credential controls (set/replace/remove its git fetch credential); the registry table itself - projects, keys, branches, remotes, statuses, poll intervals, and publication evidence - is read from the catalog database either way, and pause/resume and poll-interval changes go through the writer connection (whose role must hold the `pgokf_admin` tier). The token is environment-only by design (a CLI flag would leak it through the process listing); it is held server-side only, and a credential's secret transits exactly once (browser form to the producer): it is never persisted, logged, or rendered. A producer that does not answer surfaces an honest "unavailable" on the page, never a silent success. |
...
| `admin` | everything above, plus **Admin**: people (in `users` mode: add, change role, reset password, remove, sign out everywhere), the identity provider (in `users` mode: set up, change, switch off, remove), the MCP tokens (mint, revoke), bundles (register a directory bundle, refresh, enable or disable, retire or bring back, unregister, set the refresh cadence), and the registry (pause or resume a registered repository, set its poll interval, set or remove its git fetch credential) |
