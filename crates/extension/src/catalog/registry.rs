...
//! them. Writes (`pause`/`resume` as a status change, and the poll interval)
//! go through the `SECURITY DEFINER` functions below, granted to
//! `pgokf_admin`; no API role holds a direct write grant on the table.
