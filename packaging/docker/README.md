...
locally as below. `0.2.0` in the examples is the extension version, read from
...
| Tag             | PostgreSQL                    |
| --------------- | ----------------------------- |
| `0.2.0-pg15`   | 15 (no BM25 provider: `pg_textsearch` ships for 17 and 18 only) |
| `0.2.0-pg16`   | 16 (no BM25 provider)         |
| `0.2.0-pg17`   | 17 (+ `pg_textsearch`)        |
| `0.2.0-pg18`   | 18 (+ `pg_textsearch`; the `PG_MAJOR` default) |
| `0.2.0-pg19`   | 19 (once PGDG ships packages; no BM25 provider until Tiger Data publishes a pg19 package) |
...
docker run --rm --network pgokf-net \
  ...
  -v /srv/pgokf/backups:/backups pgokf:0.2.0-pg18 pgokf-backup
docker run --rm --network pgokf-net \
  -e PGHOST=pgokf-db -e PGUSER=postgres -e PGPASSWORD=... -e PGDATABASE=okf \
  -v /srv/pgokf/backups:/backups pgokf:0.2.0-pg18 pgokf-restore /backups/okf-<stamp>.dump
```
...
```bash
packaging/docker/smoke-test.sh pgokf:0.2.0-pg18            # local daemon
DOCKER="docker --context <remote>" packaging/docker/smoke-test.sh pgokf:0.2.0-pg18
```
...
docker run --rm pgokf-companions:0.2.0 pgokf-embed --help
packaging/docker/smoke-test-companions.sh pgokf-companions:0.2.0
```
